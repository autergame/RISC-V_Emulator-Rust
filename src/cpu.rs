...
impl RiscvCpu {
	...
	pub fn dump_registers(&self) {
		println!("\npc = {0} 0x{0:08X}", self.program_counter);
	...
	fn execute_opcode_e_and_system(&mut self, bits: u32) {
		...
		match funct3 {
			...
			FUNCT3_001 => {
				// csrrw
				let newreg = self.get_register(inst_i.rs1());
				self.set_register(inst_i.rd(), self.csrs[inst_i.imm11_0() as usize] as u32);
				self.csrs[inst_i.imm11_0() as usize] = newreg as u64;
				self.program_counter += 4;
			}
			FUNCT3_010 => {
				// csrrs
				...
				self.set_register(inst_i.rd(), self.csrs[inst_i.imm11_0() as usize] as u32);
				self.csrs[inst_i.imm11_0() as usize] =
					self.csrs[inst_i.imm11_0() as usize] | mask as u64;
				self.program_counter += 4;
			}
			FUNCT3_011 => {
				// csrrc
